...
const emit = defineEmits<{
  (e: 'submit', payload: { name: string; status: string; description: string }): void
  (e: 'close'): void
}>()
...
<template>
  ...
  >
    <form id="form-projeto" class="flex flex-col gap-4" @submit.prevent="handleFormSubmit">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <textarea 
          ...
          :class="error?.description ? 'border-red-500' : 'border-neutral-700/80'"
        ></textarea>
    ...
    <template #footer>
      <button
      aria-label="fechar criar projeto" 
        type="button" 
        @click="handleClose"
      ...
      <button 
        ...
        form="form-projeto"
        :disabled="loading"
        class="px-5 py-2.5 text-sm font-medium text-white bg-emerald-600 rounded-xl hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-950/50 active:scale-95 disabled:opacity-50 cursor-pointer"
      >
