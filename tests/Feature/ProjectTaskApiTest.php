...
class ProjectTaskApiTest extends TestCase
{
    ...
    public function test_create_a_project_and_tasks(): void
    {
        ...
        $response = $this->postJson(route('project.store'), $projectData);
        ...
        foreach ($tasks as $task) {
            $taskResponse = $this->postJson(route('task.store', ['id' => $projectId]), $task);
    ...
    public function test_list_projects_and_batch_data(): void
    {
        ...
        foreach ($projects as $project) {
            $response = $this->postJson(route('project.store'), $project);
            $response->assertStatus(201);
        }
